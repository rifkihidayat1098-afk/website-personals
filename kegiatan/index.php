...
$kegiatan = [];
$queryKegiatan = mysqli_query($koneksi, "SELECT * FROM event ORDER BY date_event DESC");
...
<html lang="id">
  <head>
    ...
    <title>Kegiatan - SMAN 2 SIMEULUE BAR</title>
                ...
                <div class="card-container">
                    <?php if (count($kegiatan) > 0): ?>
                        <?php foreach ($kegiatan as $item): ?>
                            <div class="card">
                                <a href="detail.php?id=<?= $item['id_event'] ?>" class="card-kegiatan">
                                    <img src="../uploads/event/<?= htmlspecialchars($item['img_event']) ?>" alt="gambar event">
                                    <h3><?= htmlspecialchars($item['name_event']) ?></h3>
                                    <p><?= htmlspecialchars(substr($item['detail_event'], 0, 85)) ?>...</p>
                                    <small><?= date('d M Y', strtotime($item['date_event'])) ?></small>
                                </a>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
